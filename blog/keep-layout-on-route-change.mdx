---
title: 'Next.js 拦截路由怎么用？点击弹 Modal、直达渲染详情页（附 Vue 对照实现）'
publishedAt: '2024-06-18'
summary: '对比 Next.js 拦截路由与 Vue 嵌套路由的两种实现：点击图片以 Modal 展示详情、直接访问 URL 则渲染完整页面，路由切换时保留列表页上下文，附完整示例代码。'
tags:
  - vue
  - next.js
  - 路由
...
先说答案：Next.js 用**拦截路由（Intercepting Routes）+ 平行路由**就能做到——列表页里点击图片，URL 变成详情地址但以 Modal 展示、列表上下文不丢；直接访问同一个 URL 则渲染完整的详情页。Vue 里用嵌套路由加条件渲染可以实现等价效果，下面对比两边的完整实现。
