...
下面是一个最小的进场动画。示例里的代码省略了 `import`，当前 Playground 已经把 `motion` 注入进去了。
...
function Demo() {
  return (
    <div style={{
      ...
      >
        <div style={{ fontSize: 13, opacity: 0.62, marginBottom: 8 }}>
          Motion Card
        </div>
          ...
          {open ? (
            <motion.div
              ...
              style={{
                ...
                background: 'white',
                color: '#0f172a',
                boxShadow: '0 18px 42px rgba(8, 47, 73, 0.16)'
              }}
            >
              <strong>AnimatePresence</strong>
